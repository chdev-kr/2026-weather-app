...
export const HomePage = () => {
  ...
  // 1. 현재 위치 가져오기 (GPS)
  const { latitude, longitude, error: geoError, loading: geoLoading } = useGeolocation(!currentLocation);
  ...
  const landRegId = useMemo(() => {
    ...
    return getLandForecastRegionCode(currentLocation.latitude, currentLocation.longitude);
  }, [currentLocation]);

  const tempRegId = useMemo(() => {
    if (!currentLocation) return '';
    return getTemperatureForecastRegionCode(currentLocation.latitude, currentLocation.longitude);
  }, [currentLocation]);
  ...
  const grid0 = fav0 ? convertGpsToGrid({ latitude: fav0.latitude, longitude: fav0.longitude }) : { nx: 0, ny: 0 };
  const grid1 = fav1 ? convertGpsToGrid({ latitude: fav1.latitude, longitude: fav1.longitude }) : { nx: 0, ny: 0 };
  const grid2 = fav2 ? convertGpsToGrid({ latitude: fav2.latitude, longitude: fav2.longitude }) : { nx: 0, ny: 0 };
  const grid3 = fav3 ? convertGpsToGrid({ latitude: fav3.latitude, longitude: fav3.longitude }) : { nx: 0, ny: 0 };
  const grid4 = fav4 ? convertGpsToGrid({ latitude: fav4.latitude, longitude: fav4.longitude }) : { nx: 0, ny: 0 };
  const grid5 = fav5 ? convertGpsToGrid({ latitude: fav5.latitude, longitude: fav5.longitude }) : { nx: 0, ny: 0 };

  const { data: weather0 } = useShortTermForecast(grid0, baseDate, baseTime, !!fav0);
  const { data: weather1 } = useShortTermForecast(grid1, baseDate, baseTime, !!fav1);
  const { data: weather2 } = useShortTermForecast(grid2, baseDate, baseTime, !!fav2);
  const { data: weather3 } = useShortTermForecast(grid3, baseDate, baseTime, !!fav3);
  const { data: weather4 } = useShortTermForecast(grid4, baseDate, baseTime, !!fav4);
  const { data: weather5 } = useShortTermForecast(grid5, baseDate, baseTime, !!fav5);
